...
class BookingForm(forms.ModelForm):
    ...
    time = forms.TimeField(
        widget=forms.TimeInput(attrs={'type': 'text', 'id': 'datetimepicker'}))
    guests = forms.IntegerField(
        min_value=1, max_value=28, widget=forms.NumberInput(
            attrs={'min': '1', 'max': '28', 'id': 'guests'}))
    tables = forms.ModelMultipleChoiceField(
        queryset=Table.objects.all(), widget=forms.CheckboxSelectMultiple(
            attrs={'id': 'tables', 'class': 'table-checkbox'}))
    ...
    def clean_time(self):
        ...
        The booking time must be between 12:00 and 21:00 and
        must not overlap with the restaurant's closing time.
        ...
        if booking_time and (
            booking_time < time(12, 0) or
            booking_time > time(21, 0)
        ):
            raise ValidationError(
                'Booking time must be between 12 midday and 9pm.')

        # End time is implicitly 2 hours after start time.
        end_time = (
            datetime.combine(date.today(), booking_time) + timedelta(hours=2)
        ).time()

        # Subtract one minute from the end time to
        # ensure it aligns with the 30-minute booking steps.
        end_time = (
            datetime.combine(date.today(), end_time) - timedelta(minutes=1)
        ).time()

        # End time should not be later than closing time.
        if end_time > time(21, 0):
            raise ValidationError(
                'Booking time plus duration must not exceed closing time.')
    ...
    def clean(self):
        ...
        Check for table availability and
        capacity before confirming the booking.
        ...
        if booking_time and booking_date and tables:
            for table in tables:
                ...
                if latest_booking:
                    latest_booking_end_time = (
                        datetime.combine(
                            date.today(), latest_booking.time)
                        + timedelta(hours=2)
                    ).time()
                    min_start_time = (
                        datetime.combine(date.today(), latest_booking_end_time)
                    ...
                    if booking_time < min_start_time:
                        self.add_error(
                            'time',
                            (
                                'The selected time slot overlaps with a '
                                'previous booking for Table '
                                f'{table.table_number}. Choose a time after '
                                f'{min_start_time.strftime("%H:%M")}.'
                            )
                        )
            ...
            if total_capacity < guests:
                self.add_error(
                    'guests',
                    (
                        'The total capacity of the selected '
                        'tables is insufficient '
                        'for the number of guests. Please select more tables.'
                    )
                )
